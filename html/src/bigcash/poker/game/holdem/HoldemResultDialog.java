package bigcash.poker.game.holdem;
...
import bigcash.poker.game.holdem.widgets.HoldemCard;
...
public class HoldemResultDialog extends UIDialog {
    private HoldemGameScreen gameScreen;
    private HoldemWorld holdemWorld;
    private TextureAtlas gameAtlas;
    private HoldemResultMessage resultMessage;
    ...
    public HoldemResultDialog(HoldemGameScreen gameScreen, HoldemResultMessage resultMessage) {
        super(gameScreen);
        this.gameAtlas=gameScreen.atlas;
        this.gameScreen=gameScreen;
        this.holdemWorld=gameScreen.holdemWorld;
    ...
    public void buildDialog() {
        ...
        for (HoldemResultMessage.PokerWinning winning:resultMessage.winnings){
            ...
            for (HoldemResultMessage.PokerPoolPlayer pokerPoolPlayer:winning.players){
    ...
    private class PlayerTable extends Table{
        public PlayerTable(HoldemResultMessage.PokerPoolPlayer pokerPoolPlayer,boolean allFolded){
            Table imageBackTable=new Table();
            imageBackTable.setBackground(playerDetailStyle.frameBackground);
            PokerPlayerImage playerImage=new PokerPlayerImage(playerDetailStyle.userRegion,playerDetailStyle.crownRegion,holdemWorld.maskLayer);
            pokerGame.downloadImage(pokerPoolPlayer.pokerResultPlayer.imageUrl,playerImage);
            ...
            if (pokerPoolPlayer.winState==1){
                if (allFolded){
                    ...
                    combinationLabel.setText(holdemWorld.cardRankingMap.get(pokerPoolPlayer.pokerResultPlayer.groupType));
                }
    ...
    private class PlayerCards extends Widget{
        private List<HoldemCard> holdemCards;
        public float width,height,cardPad,cardBottomPad;
        public PlayerCards(int[] cards){
            holdemCards =new ArrayList<HoldemCard>();
            if (cards==null || cards.length==0){
                HoldemCard holdemCard =new HoldemCard(holdemWorld,-1,-1);
                holdemCard.setSize(playerDetailStyle.cardWidth,playerDetailStyle.cardHeight);
                holdemCards.add(holdemCard);
                holdemCard =new HoldemCard(holdemWorld,-1,-1);
                holdemCard.setSize(playerDetailStyle.cardWidth,playerDetailStyle.cardHeight);
                holdemCards.add(holdemCard);
                holdemCard =new HoldemCard(holdemWorld,-1,-1);
                holdemCard.setSize(playerDetailStyle.cardWidth,playerDetailStyle.cardHeight);
                ...
                for (int i=0;i<cards.length;i++) {
                    HoldemCard holdemCard =new HoldemCard(holdemWorld, cards[i], 1);
                    holdemCard.setSize(playerDetailStyle.cardWidth,playerDetailStyle.cardHeight);
                    holdemCards.add(holdemCard);
                }
            }
        ...
        public void draw(Batch batch, float parentAlpha) {
            ...
            float totalWidth=cardPad*(holdemCards.size()-1)+playerDetailStyle.cardWidth;
            float startX=getX()+(getWidth()-totalWidth)/2f;
            float cardY=getY()-cardBottomPad;
            for (int i = 0; i< holdemCards.size(); i++){
                HoldemCard holdemCard = holdemCards.get(i);
                holdemCard.setPosition(startX,cardY);
                startX=startX+cardPad;
            }
            for (HoldemCard holdemCard : holdemCards){
                holdemCard.draw(batch,parentAlpha);
            }
        }
    }
    ...
    private class PlayerDetailStyle{
        ...
        public PlayerDetailStyle(){
            ...
            cardHeight=cardWidth*holdemWorld.backCardRegion.getRegionHeight()/holdemWorld.backCardRegion.getRegionWidth();
