...
public class OmahaCard extends Widget {
    public int id, rank, suit;
    private TextureRegion back, front;
    public OmahaWorld omahaWorld;

    public OmahaCard(OmahaWorld omahaWorld, int id, float scaleX) {
        this.omahaWorld=omahaWorld;
        if (id==-1){
            this.id=id;
            this.rank=id;
            this.suit=id;
            this.back=omahaWorld.backCardRegion;
            this.front=omahaWorld.backCardRegion;
        }else {
            this.id = id;
            this.rank = id % 13;
            if (this.rank == 0) {
            ...
            this.front = omahaWorld.atlas.findRegion(omahaWorld.suitMap.get(suit), rank);
            this.back = omahaWorld.backCardRegion;
        }
        this.setScaleX(scaleX);
    }

    public OmahaCard(OmahaWorld omahaWorld){
        this(omahaWorld,-1,-1.0f);
    }
