package bigcash.poker.game.poker.holdem.widgets.buttons;
...
public class CallAnyButton extends GameButton {
    ...
    public CallAnyButton(final HoldemWorld holdemWorld, int id, Label.LabelStyle labelStyle, TextureRegionDrawable uncheckedDrawable, TextureRegionDrawable checkedDrawable) {
        setName("CallAnyButton");
        ...
        NinePatchDrawable background = new NinePatchDrawable(new NinePatch(holdemWorld.atlas.findRegion("btn_callany"), 13, 13, 10, 15));
    ...
    public void updateButton(int state, float amount) {
        ...
        switch (state) {
            case UNCHECKED:
                updateButton(uncheckedDrawable, "CALL ANY");
                break;
            case CHECKED:
                updateButton(checkedDrawable, "CALL ANY");
                break;
            default:
                clearChildren();
                break;
    ...
    public boolean updateButton(boolean userTurn, boolean betPlaced, float balance, float betAmount, float maxBet) {
        if (!userTurn && betPlaced) {
            float callAmount=maxBet-betAmount;
            if (callAmount>0 && balance>0) {
        ...
        if (userTurn && betPlaced) {
            ...
            if (callAmount > 0) {
                if (balance> callAmount) {
                    ...
                    holdemWorld.screen.sendMove(PokerConstants.MOVE_ALL_IN, balance);
                    return true;
                }
            }
        }
