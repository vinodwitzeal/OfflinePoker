...
import bigcash.poker.game.poker.widgets.PokerPlayerTurnImage;
...
public class OmahaPlayer extends Group {
    ...
    public PokerTimer.PokerTimerUpdater timerUpdater;
    public PokerTimer pokerTimer;
    public float cardX, cardY, cardRotation;
    public HashMap<Integer, OmahaCard> cardHashMap;
    ...
    public OmahaPlayer(OmahaWorld omahaWorld, PokerPlayerStyle playerStyle) {
        this(omahaWorld, playerStyle, true);
    }

    public OmahaPlayer(final OmahaWorld omahaWorld, final PokerPlayerStyle playerStyle, boolean emptySeat) {
        setTransform(false);
        this.cardHashMap = new HashMap<Integer, OmahaCard>();
        this.addGiftButton = Constant.emojis!=null;
        this.omahaWorld = omahaWorld;
        this.playerStyle = playerStyle;
        this.betPadding = playerStyle.betPadding;
        this.namePadding = 3 * omahaWorld.screen.density;
        ...
        playerTurnImage = new PokerPlayerTurnImage(omahaWorld.screen,playerStyle.userRegion, playerStyle.crownRegion, playerStyle.imageSize);
        ...
        this.giftButton.addListener(new ClickListener(){
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                omahaWorld.screen.localToStageCoordinates(giftButton,position);
                position.x=position.x+playerStyle.giftSize/2f;
                position.y=position.y+playerStyle.giftSize/2f;
                omahaWorld.screen.showEmojiDialog(position.x,position.y,giftAlign,getPlayerId());
            }
        });
    ...
    public void showBetAnimation(float amount) {
        ...
        if (amount > 0) {
            omahaWorld.screen.playSound(omahaWorld.betSound, 1.0f);
            ...
            omahaWorld.addActor(amountLabel);
    ...
    public void addBetToPool() {
        if (betAmount > 0) {
            ...
            Vector2 endPosition = omahaWorld.getMainPoolPosition();
            amountLabel.setPosition(startPosition.x, startPosition.y);
            omahaWorld.addActor(amountLabel);
            amountLabel.addAction(Actions.sequence(
                    Actions.moveTo(endPosition.x, endPosition.y, 0.2f),
                    Actions.run(new Runnable() {
                        ...
                        public void run() {
                            Gdx.app.postRunnable(new Runnable() {
                                @Override
                                public void run() {
                                    omahaWorld.addToMainPool(amount);
                                    updateBetAmount();
                                }
                            });
                        }
    ...
    public Vector2 getBalancePosition() {
        return omahaWorld.screen.localToStageCoordinates(balanceLabel,new Vector2());
    }

    public Vector2 getBetPosition() {
        return omahaWorld.screen.localToStageCoordinates(betLabel,new Vector2());
    }
    ...
    private void build() {
        ...
        dealerImage.setPosition(playerTurnImage.getX(), playerTurnImage.getY() + playerTurnImage.getHeight()-dealerImage.getHeight());
        stateLabel.setSize(playerStyle.stateWidth, playerStyle.stateHeight);
        stateLabel.setAlignment(Align.center);
    ...
    public void addActors() {
        playerCards.setPosition(cardX, detailsTable.getY() + detailsTable.getHeight() / 2f);
    ...
    public void setPlayerData(PokerPlayerData playerData) {
        ...
        if (!playerData.imageUrl.isEmpty()) {
            omahaWorld.screen.pokerGame.downloadImage(playerData.imageUrl, playerTurnImage);
        }
    ...
    public void updateMoveState(String moveType) {
        ...
        if (moveType.matches(PokerConstants.MOVE_FOLD)) {
            ...
            omahaWorld.screen.playSound(omahaWorld.foldSound, 1.0f);
    ...
    public HashMap<Integer, OmahaCard> getHandCards() {
        return cardHashMap;
    }
    ...
    private class PokerPlayerCards extends WidgetGroup {
        ...
        public void updateCards(int[] cards) {
            ...
            float cardPad = playerStyle.cardWidth / 2f;

            float totalCardsWidth = playerStyle.cardWidth + cardPad * (cards.length - 1);

            float startX = -totalCardsWidth / 2f;
            float startY = cardYOffset;
            for (int i = 0; i < cards.length; i++) {
                int cardId = cards[i];
                OmahaCard omahaCard = new OmahaCard(omahaWorld, cardId, cards[0] > 0 ? 1 : -1);
                omahaCard.setSize(playerStyle.cardWidth, playerStyle.cardHeight);
                omahaCard.setOrigin(playerStyle.cardWidth / 2f, 0);
                omahaCard.setPosition(startX, startY);
                startX = startX + playerStyle.cardWidth / 2f;
                omahaCard.setRotation(cardRotation);
                omahaCard.addAction(Actions.scaleTo(1, 1, 0.2f));
                addActor(omahaCard);
                if (cardId!=-1) {
                    cardHashMap.put(cardId, omahaCard);
                }
            }
        }
        ...
        public void dealCards(int[] cards, float delay) {
            ...
            List<OmahaCard> omahaCards = new ArrayList<OmahaCard>();
            for (int i = 0; i < cards.length; i++) {
                int cardId = cards[i];
                OmahaCard omahaCard = new OmahaCard(omahaWorld, cardId, cards[0] > 0 ? 1 : -1);
                omahaCard.setSize(playerStyle.cardWidth, playerStyle.cardHeight);
                omahaCard.setOrigin(playerStyle.cardWidth / 2f, 0);
                omahaCard.setScale(-1, 1);
                omahaCard.setPosition(startPosition.x, startPosition.y);
            ...
            float startX = -totalCardsWidth / 2f;
            float startXRotation = 8;
            float startY = cardYOffset;
            float rotation = 16f / cardHashMap.size();
            for (OmahaCard omahaCard : omahaCards) {
                omahaCard.addAction(Actions.sequence(
                        Actions.delay(delay),
                        Actions.moveTo(startX, startY, 0.3f),
                        Actions.scaleTo(1, 1, 0.1f),
                        Actions.rotateTo(cardRotation)));
                startX = startX + cardPad;
                startXRotation = startXRotation - rotation;
                addActor(omahaCard);
            }
        }
    ...
    public static class PokerPlayerStyle {
        ...
        public TextureRegionDrawable detailsBackground;
        ...
        public float giftSize;
        public TextureRegionDrawable giftDrawable;

        public PokerPlayerStyle(OmahaWorld omahaWorld, MaskLayer maskLayer) {
            float screenWidth = omahaWorld.screen.width;
            float screenHeight = omahaWorld.screen.height;
            this.maskLayer = maskLayer;

            TextureAtlas atlas = omahaWorld.atlas;
            ...
            TextureRegion cardRegion = omahaWorld.backCardRegion;
            ...
            giftDrawable= new TextureRegionDrawable(omahaWorld.screen.uiAtlas.findRegion("icon_gift"));
        }

        public PokerPlayerStyle(TextureRegion userRegion, PokerPlayerStyle pokerPlayerStyle) {
            this.maskLayer = pokerPlayerStyle.maskLayer;
            ...
            giftSize=pokerPlayerStyle.giftSize;
            giftDrawable=pokerPlayerStyle.giftDrawable;
        }
    }
